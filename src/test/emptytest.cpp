...
TESTS_INIT()
    rbb::object empty_equals_empty = rbb::empty().send_msg(rbb::symbol("==")).send_msg(rbb::empty());
    ...
    rbb::object empty_differs_empty = rbb::empty().send_msg(rbb::symbol("!=")).send_msg(rbb::empty());
    
    TEST_CONDITION(
        empty_differs_empty == rbb::boolean(false),
        puts("\"Empty differs empty\" equals true"))
    TEST_CONDITION(
        rbb::empty().send_msg(rbb::symbol("==")).send_msg(rbb::symbol("duvnis")) == rbb::boolean(false),
